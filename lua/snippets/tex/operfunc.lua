...
mysnips[#mysnips+1] = s( { trig="pa([A-Za-z])([A-Za-z])2", regTrig=true }, fmta([[\frac{\partial^{2} <>}{\partial <>^{2}}]],
{ f(function(args, snip) return snip.captures[1] end, {}), f(function(args, snip) return snip.captures[2] end, {}) }), { condition = math })

mysnips[#mysnips+1] = s( { trig="ddt", wordTrig=false, snippetType="autosnippet" }, t([[\frac{\mathrm{d}}{\mathrm{d}t}]]), { condition = math })

mysnips[#mysnips+1] = s( { trig="de([A-Za-z])([A-Za-z])", regTrig=true }, fmta([[\frac{\mathrm{d}<>}{\mathrm{d}<>}]],
{ f(function(args, snip) return snip.captures[1] end, {}), f(function(args, snip) return snip.captures[2] end, {}) }), { condition = math })
mysnips[#mysnips+1] = s( { trig="de([A-Za-z])([A-Za-z])2", regTrig=true }, fmta([[\frac{\mathrm{d}^{2}<>}{\mathrm{d}<>^{2}}]],
{ f(function(args, snip) return snip.captures[1] end, {}), f(function(args, snip) return snip.captures[2] end, {}) }), { condition = math })

local trig = { "sin", "cos", "tan", "cot", "csc", "sec", "arccsc", "arcsec"}
local htrig = { "sinh", "cosh", "tanh", "coth" }

for index = 1, #trig, 1 do
	mysnips[#mysnips+1] = s({ trig=trig[index], wordTrig=false, snippetType="autosnippet" }, fmta("\\"..trig[index], {}), { condition = math })
	mysnips[#mysnips+1] = s({ trig="\\"..trig[index].."([A-Za-gi-z0-9])", regTrig=true, wordTrig=false, snippetType="autosnippet" }, fmta("\\"..trig[index].." <>",
	{ f(function(args, snip) return snip.captures[1] end, {}) }), { condition = math })
end

for index = 1, #htrig, 1 do
	mysnips[#mysnips+1] = s({ trig="\\"..htrig[index].."(%w)", regTrig=true, wordTrig=false, snippetType="autosnippet" }, fmta("\\"..htrig[index].." <>",
	{ f(function(args, snip) return snip.captures[1] end, {}) }), { condition = math })
end
